...
ALTER TABLE material
    ADD CONSTRAINT pk_material PRIMARY KEY ( sklad_materialu,
                                             id_materialu,
                                             zakazka );
...
ALTER TABLE seznam_techniky
    ADD CONSTRAINT pk_sez_tech PRIMARY KEY ( id_seznamu_techniky,
                                             sklad_techniky,
                                             zakazka );
...
CREATE UNIQUE INDEX i_sidlo_ ON
    sidlo (
        typ_sidla
    ASC );
...
ALTER TABLE material
    ADD CONSTRAINT fk_material_zakazka FOREIGN KEY ( zakazka )
...
ALTER TABLE seznam_techniky
    ADD CONSTRAINT fk_sez_tech_zakazka FOREIGN KEY ( zakazka )
        REFERENCES zakazka ( id_zakazky );
...
CREATE SEQUENCE typ_sidla_id_nazev_typu_seq START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER typ_sidla_id_nazev_typu_trg BEFORE
    INSERT ON typ_sidla
    FOR EACH ROW
    WHEN ( new.id_nazev_typu IS NULL )
BEGIN
    :new.id_nazev_typu := typ_sidla_id_nazev_typu_seq.nextval;
END;
/
